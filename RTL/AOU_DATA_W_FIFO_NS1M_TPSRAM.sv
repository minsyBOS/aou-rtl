...
// *****************************************************************************
//
//  Module     : AOU_DATA_W_FIFO_NS1M_TPSRAM
//  Version    : 1.00
//  Date       : 26-02-26
...
#(
    ...
    localparam  AXI_STRB_FIFO_WD                 = AXI_PEER_DIE_MAX_DATA_WD/8,
    ...
    localparam  CTRL_FIFO_WD                = 2 + 1, //DLEN + wdataf
...
(
    ...
    input  [ICH_CNT-1:0][AXI_STRB_FIFO_WD-1:0]                   I_SDATA_STRB,
    input  [ICH_CNT-1:0]                                    I_SDATA_WDATAF,
    output                                                  O_SREADY,
    
    input                                                   I_MREADY,
    output [AXI_PEER_DIE_MAX_DATA_WD-1:0]                   O_MDATA,
    output [AXI_STRB_FIFO_WD-1:0]                                O_MDATA_STRB,
    output [1:0]                                            O_MDLEN,
    ...
    input   [REQ_CNT-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]    I_FIFO_SRAM_RD_DATA,

    output  [REQ_CNT-1:0]                                   O_FIFO_SRAM_WR_ENB,
    output  [REQ_CNT-1:0][DATA_AW-1:0]                      O_FIFO_SRAM_WR_ADDR,
    output  [REQ_CNT-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]    O_FIFO_SRAM_WR_DATA
...
//--------------------sram-------------------------------------------
    ...
    logic [REQ_CNT-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]              sram_wr_data;
    
    logic                                                           w_sram_dout_valid;
    logic [REQ_CNT-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]              w_sram_dout;
    logic                                                           r_sram_dout_d1_valid;
    logic [REQ_CNT-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]              r_sram_dout_d1;
...
//-------------------regfifo--------------------------------------
    logic [AXI_CON-1:0][DATA_FIFO_WD+STRB_FIFO_WD-1:0]              w_regfifo_input_data;

    logic [ICH_CNT-1:0]                                             w_regfifo_svalid;
    logic [ICH_CNT-1:0][AXI_PEER_DIE_MAX_DATA_WD+AXI_STRB_FIFO_WD-1:0]   w_regfifo_sdata;
    logic                                                           w_regfifo_sready;

    logic                                                           w_regfifo_mready;
    logic [AXI_PEER_DIE_MAX_DATA_WD+AXI_STRB_FIFO_WD-1:0]                w_regfifo_mdata;
        ...
        for(int dec_multi = 0; dec_multi < DEC_MULTI; dec_multi = dec_multi + 1) begin
            if(~cen_b_tmp[dec_multi*4+0]) begin
                for(int sram_cnt = 0; sram_cnt < 4; sram_cnt = sram_cnt + 1) begin
                    ...
                    sram_wr_data[next_wr_sram_idx] = {I_SDATA[dec_multi*4+0][(DATA_FIFO_WD*sram_cnt) +: DATA_FIFO_WD], I_SDATA_STRB[dec_multi*4+0][(STRB_FIFO_WD*sram_cnt) +: STRB_FIFO_WD]};
                ...
                for(int sram_cnt = 0; sram_cnt < 2; sram_cnt = sram_cnt + 1) begin
                    next_wr_sram_idx = ((wr_sram_idx + write_req_cnt) >= REQ_CNT) ? (wr_sram_idx + write_req_cnt - REQ_CNT) : (wr_sram_idx + write_req_cnt);
                    nxt_wr_sram_idx_valid[next_wr_sram_idx] = 1'b1;
                    sram_wr_data[next_wr_sram_idx] = {I_SDATA[dec_multi*4+1][(DATA_FIFO_WD*sram_cnt) +: DATA_FIFO_WD], I_SDATA_STRB[dec_multi*4+1][(STRB_FIFO_WD*sram_cnt) +: STRB_FIFO_WD]};
                    write_req_cnt = write_req_cnt + 1;
                end
            end
            if(~cen_b_tmp[dec_multi*4+2]) begin
                next_wr_sram_idx = ((wr_sram_idx + write_req_cnt) >= REQ_CNT) ? (wr_sram_idx + write_req_cnt - REQ_CNT) : (wr_sram_idx + write_req_cnt);
                nxt_wr_sram_idx_valid[next_wr_sram_idx] = 1'b1;
                sram_wr_data[next_wr_sram_idx] = {I_SDATA[dec_multi*4+2][DATA_FIFO_WD-1:0], I_SDATA_STRB[dec_multi*4+2][STRB_FIFO_WD-1:0]};
                write_req_cnt = write_req_cnt + 1;
            end
            if(~cen_b_tmp[dec_multi*4+3]) begin
                next_wr_sram_idx = ((wr_sram_idx + write_req_cnt) >= REQ_CNT) ? (wr_sram_idx + write_req_cnt - REQ_CNT) : (wr_sram_idx + write_req_cnt);
                nxt_wr_sram_idx_valid[next_wr_sram_idx] = 1'b1;
                sram_wr_data[next_wr_sram_idx] = {I_SDATA[dec_multi*4+3][DATA_FIFO_WD-1:0], I_SDATA_STRB[dec_multi*4+3][STRB_FIFO_WD-1:0]};
                write_req_cnt = write_req_cnt + 1; 
            end   
        end
...
//-------------extra fifo flow control signals----------------------------
    always_comb begin
        ...
        for(int dec_multi = 0; dec_multi < DEC_MULTI; dec_multi = dec_multi + 1) begin       
            if(I_SVALID[dec_multi*4+0]) begin
                nxt_ctrl_data[ctrl_req_cnt] = {2'b10, I_SDATA_WDATAF[dec_multi*4+0]};
                ...
                nxt_ctrl_data[ctrl_req_cnt] = {2'b01, I_SDATA_WDATAF[dec_multi*4+1]};
                nxt_w_ctrl_ptr[ctrl_req_cnt] = ((w_ctrl_ptr + ctrl_req_cnt) >= CTRL_FIFO_DEPTH) ? ((w_ctrl_ptr + ctrl_req_cnt) - CTRL_FIFO_DEPTH) : (w_ctrl_ptr + ctrl_req_cnt);
                ctrl_req_cnt = ctrl_req_cnt + 1;
            end
            if(I_SVALID[dec_multi*4+2]) begin
                nxt_ctrl_data[ctrl_req_cnt] = {2'b00, I_SDATA_WDATAF[dec_multi*4+2]};
                nxt_w_ctrl_ptr[ctrl_req_cnt] = ((w_ctrl_ptr + ctrl_req_cnt) >= CTRL_FIFO_DEPTH) ? ((w_ctrl_ptr + ctrl_req_cnt) - CTRL_FIFO_DEPTH) : (w_ctrl_ptr + ctrl_req_cnt);
                ctrl_req_cnt = ctrl_req_cnt + 1;
            end
            if(I_SVALID[dec_multi*4+3]) begin
                nxt_ctrl_data[ctrl_req_cnt] = {2'b00, I_SDATA_WDATAF[dec_multi*4+3]};
                nxt_w_ctrl_ptr[ctrl_req_cnt] = ((w_ctrl_ptr + ctrl_req_cnt) >= CTRL_FIFO_DEPTH) ? ((w_ctrl_ptr + ctrl_req_cnt) - CTRL_FIFO_DEPTH) : (w_ctrl_ptr + ctrl_req_cnt);
                ctrl_req_cnt = ctrl_req_cnt + 1;
            end
        end
    ...
    assign o_mdlen  = ctrl_fifo[r_ctrl_sram_ptr][2:1];
    assign o_mdlen_cen_a = (w_regfifo_sready & (r_sram_dout_d1_valid | ~r_cen_a_d1)) ? ctrl_fifo[nxt_r_ctrl_sram_ptr_cen_a][2:1] : ctrl_fifo[r_ctrl_sram_ptr][2:1];

    assign O_MDLEN          = ctrl_fifo[r_ctrl_ptr][2:1];
    assign O_MDATA_WDATAF   = ctrl_fifo[r_ctrl_ptr][0];
...
//------regfifo------------------------------------------------------
    genvar u;
    generate
        for(u = 0; u < ICH_CNT; u = u + 1) begin 
            assign w_regfifo_svalid[u]  = (bypass_sramfifo[u]) ? I_SVALID[u] : (u == 0) ? w_sramfifo_mvalid : 1'b0;
            assign w_regfifo_sdata[u]   = (bypass_sramfifo[u]) ? {I_SDATA[u], I_SDATA_STRB[u]} : 
                                                                 (u == 0) ? (o_mdlen == 2) ? {w_regfifo_input_data[3][STRB_FIFO_WD +: DATA_FIFO_WD], w_regfifo_input_data[2][STRB_FIFO_WD +: DATA_FIFO_WD], w_regfifo_input_data[1][STRB_FIFO_WD +: DATA_FIFO_WD], w_regfifo_input_data[0][STRB_FIFO_WD +: DATA_FIFO_WD], 
                                                                                              w_regfifo_input_data[3][STRB_FIFO_WD-1:0], w_regfifo_input_data[2][STRB_FIFO_WD-1:0], w_regfifo_input_data[1][STRB_FIFO_WD-1:0], w_regfifo_input_data[0][STRB_FIFO_WD-1:0]} :
                                                                            (o_mdlen == 1) ? {{(2*(DATA_FIFO_WD)){1'b0}}, w_regfifo_input_data[1][STRB_FIFO_WD +: DATA_FIFO_WD], w_regfifo_input_data[0][STRB_FIFO_WD +: DATA_FIFO_WD], {(2*(STRB_FIFO_WD)){1'b0}}, w_regfifo_input_data[1][STRB_FIFO_WD-1:0], w_regfifo_input_data[0][STRB_FIFO_WD-1:0]} :
                                                                                             {{(3*(DATA_FIFO_WD)){1'b0}}, w_regfifo_input_data[0][STRB_FIFO_WD +: DATA_FIFO_WD], {(3*(STRB_FIFO_WD)){1'b0}}, w_regfifo_input_data[0][STRB_FIFO_WD-1:0]} : {(AXI_PEER_DIE_MAX_DATA_WD + AXI_STRB_FIFO_WD){1'b0}};
        end
    endgenerate
    
    assign w_regfifo_mready     = I_MREADY;
    assign O_MDATA              = w_regfifo_mdata[AXI_PEER_DIE_MAX_DATA_WD + AXI_STRB_FIFO_WD-1:AXI_STRB_FIFO_WD];
    assign O_MDATA_STRB         = w_regfifo_mdata[AXI_STRB_FIFO_WD-1:0];
    assign O_MVALID             = w_regfifo_mvalid;

    AOU_SYNC_FIFO_NS1M_SREADY#(
        .FIFO_WIDTH         (AXI_PEER_DIE_MAX_DATA_WD + AXI_STRB_FIFO_WD),
